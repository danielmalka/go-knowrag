...
// TestValidateModes_DryRunCannotPruneOrReportJSON pins the two combinations this build cannot
// honour, and pins them as refusals rather than as no-ops.
//
// A dry run returns from runIngest before ingest.Orchestrate, so it holds no index snapshot: --prune
// would delete nothing while reading like it pruned, and --json would print the dry run's human
// lines to a caller that asked for machine output. Both are silent failures — which is why the
// combination errors instead of being ignored.
func TestValidateModes_DryRunCannotPruneOrReportJSON(t *testing.T) {
	refused := map[string]ingestOptions{
		"--dry-run --prune": {dryRun: true, prune: true},
		"--dry-run --json":  {dryRun: true, json: true},
	}
	for name, opts := range refused {
		t.Run(name, func(t *testing.T) {
			err := validateModes(opts)
			if !errors.Is(err, errUsage) {
				t.Fatalf("validateModes(%+v) = %v, want errUsage", opts, err)
			}
			if !strings.Contains(err.Error(), "--dry-run") {
				t.Errorf("error %q does not name both flags of the combination", err)
			}
		})
	}
	...
	accepted := map[string]ingestOptions{
		...
		"a real run with both":        {prune: true, json: true, yes: true},
	}
...
// TestIngestCmd_DryRunHelp_SaysItListsNoOrphans is a test about a sentence, and it is here because
// the sentence is the only thing standing between an operator and a wrong conclusion: a dry run
// reads nothing from Qdrant, so its clean output says nothing at all about deleted notes. Drop the
// caveat from the help and nothing else in the suite notices.
func TestIngestCmd_DryRunHelp_SaysItListsNoOrphans(t *testing.T) {
	usage := newIngestCmd(&config.Config{}).Flags().Lookup("dry-run").Usage
	for _, want := range []string{"never reads the index", "prune candidates"} {
		if !strings.Contains(usage, want) {
			t.Errorf("--dry-run usage %q does not warn that %q", usage, want)
		}
	}
}
...
// never produces (batch.go sets both together). Written the honest way, with an empty list, the test
// would pass with the guard deleted, because ingest.Prune over nothing also deletes nothing and also
// returns 0. The impossible state is what makes the store's call log discriminate: with the guard
// gone, that list is one DeleteByFilter.
func TestPruneOrphans_SnapshotUnavailable_RefusesWithoutTouchingTheStore(t *testing.T) {
	...
	err := pruneOrphans(t.Context(), store, defaultTenantID, nil, &report)
	if err == nil {
...
func TestPruneOrphans_Scanned_PrunesAndCountsWhatItRemoved(t *testing.T) {
	...
	if err := pruneOrphans(t.Context(), store, defaultTenantID, roots, &report); err != nil {
...
func TestPruneOrphans_NoteStillOnDisk_IsNotDeleted(t *testing.T) {
	...
	err := pruneOrphans(t.Context(), store, defaultTenantID,
		map[string]string{"trabalho": root}, &report)
	if err != nil {
